...
void TreeInsertHelp(rb_red_blk_tree* tree, rb_red_blk_node* z) {
	...
	((PIECE*)z->info)->size_left = abs_off_to_size_left((PIECE*)z->info);

	((PIECE*)z->info)->flags &= ~ABS_OFF;

	fix_sizes((PIECE*)z->info, ((PIECE*)z->info)->size_left);*/
...
void insert_fixup(rb_red_blk_tree *tree, rb_red_blk_node *node)
{
	rb_red_blk_node *y;
	node->red=1;
	while(node->parent->red) { /* use sentinel instead of checking for root */
...
/*insert a new node to the left of node and return it*/
rb_red_blk_node *insert_left(rb_red_blk_tree *tree, rb_red_blk_node *node)
{
	rb_red_blk_node *nil = tree->nil;
	rb_red_blk_node *ret = (rb_red_blk_node*)malloc(sizeof(rb_red_blk_node));

	memset(ret, 0, sizeof(rb_red_blk_node));
	...
	insert_fixup(tree, ret);
	return ret;
}

/*insert a new node to the left of node and return it*/
rb_red_blk_node *insert_right(rb_red_blk_tree *tree, rb_red_blk_node *node)
{
	rb_red_blk_node *nil = tree->nil;
	rb_red_blk_node *ret = (rb_red_blk_node*)malloc(sizeof(rb_red_blk_node));
...
rb_red_blk_node *TreeMin(rb_red_blk_tree *tree)
{
	rb_red_blk_node *ret = tree->root->left;
	while (ret->left != tree->nil)
		ret = ret->left;
	return ret;
}

rb_red_blk_node *TreeMax(rb_red_blk_tree *tree)
{
	rb_red_blk_node *ret = tree->root->left;
